...
> **Implementation correction (2026-07-25):** Sections 1-8 preserve the
> historical gap analysis; they do not authorize the current experimental
> compositor as retail-equivalent. `OPENOMEGA_ENABLE_RETAIL_FRONT_END` now
> enables a decoded-data preview only when combined with
> `--developer-diagnostics`. Its DFS traversal, IE/GUI interleave,
> submission-order depth, text-last ordering, and one-tick-per-rendered-frame
> cadence are explicit PROJECT preview policies. `RetailRequired` remains
> `PresentationUnavailable` even after a preview frame publishes; decoded-data
> provenance cannot authorize unevidenced presentation behavior.
