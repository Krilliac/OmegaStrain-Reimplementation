...
//
// Format .prn - tracked aggregate occurrences: recursive-in-HOG = 1, top-level-HOG = 1.
// Tracked STRUCTURAL evidence for this format: NONE. tools/fingerprint_assets.py has no handler for
// .prn, and no analysis/formats/*.md documents its layout, so no field, offset, count, header,
...
//   1. Run tools/measure_member_structural_fingerprint.py (or add a fingerprint_prn handler to
...
//   3. Move this header to native/include/omega/retail/prn_container_descriptor.h, add
//      native/src/retail/prn_container_descriptor.cpp implementing InspectPrnContainer as a
//      bounded, fail-closed, allocation-checked reader honoring DecodeLimits.
//   4. Register both in CMakeLists.txt and add native/tests/prn_container_descriptor_tests.cpp
...
{
// Passive structural descriptor placeholder for the .prn member family. Opaque until tracked
// evidence proves internal structure; no semantics are assigned to any payload byte.
struct PrnContainerDescriptor
{
...
[[nodiscard]] asset::DecodeResult<PrnContainerDescriptor> InspectPrnContainer(
