...
//
// Format .skel - tracked aggregate occurrences: recursive-in-HOG = 4, top-level-HOG = 4.
// Tracked STRUCTURAL evidence for this format: NONE. tools/fingerprint_assets.py has no handler for
// .skel, and no analysis/formats/*.md documents its layout, so no field, offset, count, header,
...
//   1. Run tools/measure_member_structural_fingerprint.py (or add a fingerprint_skel handler to
...
//   3. Move this header to native/include/omega/retail/skel_container_descriptor.h, add
//      native/src/retail/skel_container_descriptor.cpp implementing InspectSkelContainer as a
//      bounded, fail-closed, allocation-checked reader honoring DecodeLimits.
//   4. Register both in CMakeLists.txt and add native/tests/skel_container_descriptor_tests.cpp
...
{
// Passive structural descriptor placeholder for the .skel member family. Opaque until tracked
// evidence proves internal structure; no semantics are assigned to any payload byte.
struct SkelContainerDescriptor
{
...
[[nodiscard]] asset::DecodeResult<SkelContainerDescriptor> InspectSkelContainer(
