...
//
// Format .fnt - tracked aggregate occurrences: recursive-in-HOG = 3, top-level-HOG = 3.
// Tracked STRUCTURAL evidence for this format: NONE. tools/fingerprint_assets.py has no handler for
// .fnt, and no analysis/formats/*.md documents its layout, so no field, offset, count, header,
...
//   1. Run tools/measure_member_structural_fingerprint.py (or add a fingerprint_fnt handler to
...
//   3. Move this header to native/include/omega/retail/fnt_container_descriptor.h, add
//      native/src/retail/fnt_container_descriptor.cpp implementing InspectFntContainer as a
//      bounded, fail-closed, allocation-checked reader honoring DecodeLimits.
//   4. Register both in CMakeLists.txt and add native/tests/fnt_container_descriptor_tests.cpp
...
{
// Passive structural descriptor placeholder for the .fnt member family. Opaque until tracked
// evidence proves internal structure; no semantics are assigned to any payload byte.
struct FntContainerDescriptor
{
...
[[nodiscard]] asset::DecodeResult<FntContainerDescriptor> InspectFntContainer(
