...
//
// Format .txt - tracked aggregate occurrences: recursive-in-HOG = 3, top-level-HOG = 0.
// Tracked STRUCTURAL evidence for this format: NONE. tools/fingerprint_assets.py has no handler for
// .txt, and no analysis/formats/*.md documents its layout, so no field, offset, count, header,
...
//   1. Run tools/measure_member_structural_fingerprint.py (or add a fingerprint_txt handler to
...
//   3. Move this header to native/include/omega/retail/txt_container_descriptor.h, add
//      native/src/retail/txt_container_descriptor.cpp implementing InspectTxtContainer as a
//      bounded, fail-closed, allocation-checked reader honoring DecodeLimits.
//   4. Register both in CMakeLists.txt and add native/tests/txt_container_descriptor_tests.cpp
...
{
// Passive structural descriptor placeholder for the .txt member family. Opaque until tracked
// evidence proves internal structure; no semantics are assigned to any payload byte.
struct TxtContainerDescriptor
{
...
[[nodiscard]] asset::DecodeResult<TxtContainerDescriptor> InspectTxtContainer(
