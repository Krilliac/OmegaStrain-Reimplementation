...
The initial `omega_frontend_presentation` entry point deliberately has no clock or mutable screen
state. An animated retail screen cannot pass that entry point. A later live-presentation contract
...
alphanumeric string, which let a mistyped level reach content I/O before failing. Once configuration
is valid, a direct root and its optional direct level replace that whole tuple; the configured level
is never inherited. Programmatically inconsistent direct options return defensive `invalid-options`.
