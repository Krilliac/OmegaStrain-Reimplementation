...
## Front-end phase/draw evidence contract and wire mirror (E-0128, 2026-07-23)

- The corrected evidence contract is `OMEGAFRPHASE0002`. The detailed normalized trace is an
  explicitly selected ignored private output, while the separate default public report contains
  only fixed terminal/completeness/policy categories and eight bounded aggregate counts. Invocation
  forests, sites, event/submission/draw chronology, memberships, frame sequences, capture values,
  and digests never enter the public report.
- The private wire now carries an anonymous site table, `invocation.site`, one fixed lifecycle lane,
  dense global event/submission/final-draw chronology, immutable submission-time invocation context,
  contiguous primitive ranges, exact submitted/skipped draw dispositions, total submission-to-draw
  memberships, terminal failure accounting, commit/abort state, and discovered-versus-retained
  counts. Validation rejects lifecycle crossings/orphans, stack violations, temporal gaps, draws
...
- Every input/table/nesting/work/scratch/output capacity is caller-tightenable below an immutable
  ...
  failed pathname.
- E-0128 adds a standalone test-only C++ encoder that independently constructs the generated v2
  model as owned bytes and compares it with the exact 495-byte tracked fixture. It has no OpenOmega
  library dependency, exported header, shipping edge, decoder, canonical-IR type, runtime consumer,
  or owner-data integration. A shared ASCII synthetic-contract file makes the magic, header/table
  metadata, fixture size, and all twenty immutable hard limits mechanically comparable from the C++
  and Python tests. CMake registration is present; local C++ compilation and execution remain
  pending until the host preflight permits a serialized build.
- Build-independent validation passed the 55-test focused phase suite, all 450 tooling tests, Python
  compile-all, the 390-file native-dependency gate, both 128-record ledger gates, the 595-blob
  staged public-tree gate, DCO for all 54 branch commits, privacy scans, and diff checks. The required
  preflight remained `STOP` at 4.67 GiB of kernel pool, so no local C++ build was attempted.
- The mirror is an independent implementation check over project-generated bytes, not a producer or
  independent retail evidence. The scene/front-end artifacts and PCSX2 phase producer must still
