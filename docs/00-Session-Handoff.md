...
## Branch-local front-end phase/draw evidence contract correction (2026-07-23)

- The corrected Python-only contract is `OMEGAFRPHASE0002`. The detailed normalized trace is an
...
- The generated v2 fixture and Python tests establish validator/reducer self-consistency only. A
  ...
  immutable submission context. No producer, capture, named phase, grounded layer/text/animation
  order, retail semantic, frame parity, or PCSX2 equivalence is claimed.
- This branch deliberately has no evidence-ledger entry. `E-0127` is assigned elsewhere; allocate a
  fresh evidence ID only after rebasing on the current integration head and describe implementation
  facts rather than semantic attribution.
