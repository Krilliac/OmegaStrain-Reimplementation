...
`InspectSkmContainer`, `InspectSkaContainer`, and `DecodeSkasTextEnvelope` all explicitly retain no
payload bytes and assign no such meaning. Wave C of the campaign queue nonetheless calls for a
...
weight encoding, or normalization rule is asserted. `PoseIR` is one local (joint-to-parent)
transform per joint in skeleton source order; `WorldPoseIR` is the corresponding accumulated
joint-to-world result. `ClipIR`/`ClipKeyframeIR` exist only as a bounded owned target shape for a
...
length, and every counted component charged against the caller's `DecodeLimits::maximum_items`
under checked (overflow-safe) arithmetic. They assign no retail semantics; they only gate what is
safe to evaluate.

`omega::runtime::EvaluateBindPose` and `omega::runtime::EvaluatePose`
(`native/include/omega/runtime/model_pose_evaluation.h`,
`native/src/runtime/model_pose_evaluation.cpp`) compose `world[i] = world[parent[i]] * local[i]`
...
functions independently re-check every parent index and transform for validity regardless of
...
- Establishing a retail clip/keyframe grammar, sample-time unit, interpolation rule, or blend
  policy.
- VFS-facing model-member discovery, GPU upload, or any renderer/gameplay integration of `ModelIR`.
