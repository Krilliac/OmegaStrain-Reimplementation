...
// All-or-nothing admission for one decoded batch: either every frame in the
// batch is admissible - and the caller receives the per-frame normalized
// timestamps plus the state to adopt - or the batch is rejected as a whole and
// nothing about it is admitted. The input state is read, never advanced, so a
// rejected batch cannot move the queue count or either normalization cursor.
// The player relies on this to keep a rejected batch out of its future-frame
// queue entirely instead of queueing the frames preceding the rejection.
//
...
public:
  ...
  //
  // A decoded batch is admitted all-or-nothing (see
  // detail::ValidateOpeningMovieFrameBatch). FrameExtentChanged,
  // InvalidTimestamp and FrameQueueExceeded therefore leave the future-frame
  // queue, the published frame and both normalization cursors exactly as the
  // rejected batch found them - no frame preceding the rejection is queued -
  // before the player is marked Failed. The contract does not depend on this
  // failure being terminal.
