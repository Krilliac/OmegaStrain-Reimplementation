...
ValidateOpeningMovieFrameBatch(
  ...
  // Every decision reads and advances this local copy, so the caller's state is
  // reached only through the returned value and only after the whole batch has
  // been admitted.
  ...
  for (const OpeningMovieDecodedFrameFacts &frame : batch) {
    ...
    if (duration > std::numeric_limits<std::uint64_t>::max() - normalized)
      return std::unexpected(OpeningMoviePlayerErrorCode::InvalidTimestamp);
    if (admission.state.queued_frame_count >= state.maximum_queued_frames)
...
struct OpeningMoviePlayer::Impl {
  ...
  // All-or-nothing batch admission. The whole batch is validated against a copy
  // of the queue state before a single frame is moved out of it, so a rejected
  // batch leaves the caller's frames intact, the future-frame queue unchanged,
  // and both normalization cursors where the batch found them; no frame ahead
  // of the rejection is queued. The commit pass below cannot reject anything -
  // only allocation can interrupt it, and Advance turns that into the terminal
  // AllocationFailed failure.
  [[nodiscard]] std::optional<Error>
