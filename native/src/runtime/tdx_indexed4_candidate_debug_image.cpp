...
namespace {
constexpr std::uint64_t kPaletteEntries = 16U;
...
[[nodiscard]] constexpr TdxIndexed4CandidateDebugImageError
Error(const TdxIndexed4CandidateDebugImageErrorCode code) noexcept {
  return TdxIndexed4CandidateDebugImageError{
      .code = code,
      .message = TdxIndexed4CandidateDebugImageErrorMessage(code),
  };
}
...
IsValidNibbleOrder(const TdxIndexed4NibbleOrderCandidate candidate) noexcept {
  switch (candidate) {
  case TdxIndexed4NibbleOrderCandidate::LowNibbleFirst:
  case TdxIndexed4NibbleOrderCandidate::HighNibbleFirst:
    return true;
  }
  return false;
}

[[nodiscard]] constexpr bool IsValidPalettePermutation(
    const TdxIndexed4PalettePermutationCandidate candidate) noexcept {
  switch (candidate) {
  case TdxIndexed4PalettePermutationCandidate::SourceOrderIdentity:
    return true;
  }
  return false;
}
...
[[nodiscard]] constexpr bool IsValidSourceChannels(
    const TdxIndexed4SourceChannelCandidate candidate) noexcept {
  switch (candidate) {
...
[[nodiscard]] constexpr bool
IsValidAlpha(const TdxIndexed4AlphaCandidate candidate) noexcept {
  switch (candidate) {
  case TdxIndexed4AlphaCandidate::Opaque:
  case TdxIndexed4AlphaCandidate::SourceSlot3:
  case TdxIndexed4AlphaCandidate::SourceSlot3TimesTwoClamped:
    return true;
  }
  return false;
}
...
[[nodiscard]] constexpr bool
IsValidSourceLayout(const TdxIndexed4SourceLayoutCandidate candidate) noexcept {
  switch (candidate) {
  case TdxIndexed4SourceLayoutCandidate::LinearRowsTopToBottom:
  case TdxIndexed4SourceLayoutCandidate::LinearRowsBottomToTop:
    return true;
  }
  return false;
}
...
[[nodiscard]] std::expected<ImagePlan, TdxIndexed4CandidateDebugImageError>
Preflight(const asset::TextureStorageIR &storage,
          const TdxIndexed4CandidatePolicy &policy,
          const TdxIndexed4CandidateDebugImageLimits &limits) noexcept {
  if (!IsValidNibbleOrder(policy.nibble_order)) {
    return std::unexpected(Error(
        TdxIndexed4CandidateDebugImageErrorCode::InvalidNibbleOrderCandidate));
  }
  if (!IsValidPalettePermutation(policy.palette_permutation)) {
    return std::unexpected(Error(TdxIndexed4CandidateDebugImageErrorCode::
  ...
  if (!IsValidAlpha(policy.alpha_mapping)) {
    return std::unexpected(
        Error(TdxIndexed4CandidateDebugImageErrorCode::InvalidAlphaCandidate));
  }
  ...
  if (limits.maximum_source_bytes >
    ...
    return std::unexpected(
        Error(TdxIndexed4CandidateDebugImageErrorCode::InvalidLimits));
  }
  ...
  if (!IsValidSampleEncoding(storage.sample_encoding)) {
    return std::unexpected(
        Error(TdxIndexed4CandidateDebugImageErrorCode::InvalidSampleEncoding));
  }
  if (storage.sample_encoding != asset::TextureSampleEncoding::Indexed4) {
    return std::unexpected(Error(
        TdxIndexed4CandidateDebugImageErrorCode::UnsupportedSampleEncoding));
  }
  if (storage.blocks.size() != 1U) {
    return std::unexpected(
        Error(TdxIndexed4CandidateDebugImageErrorCode::BlockCountMismatch));
  }

  const asset::TextureStorageBlockIR &block = storage.blocks.front();
  if (block.planes.size() != 1U) {
    return std::unexpected(
        Error(TdxIndexed4CandidateDebugImageErrorCode::PlaneCountMismatch));
  }
  if (!block.palette) {
    return std::unexpected(
        Error(TdxIndexed4CandidateDebugImageErrorCode::MissingPalette));
  }

  const asset::TextureStoragePlaneIR &plane = block.planes.front();
  if (plane.width == 0U || plane.height == 0U) {
    return std::unexpected(
        Error(TdxIndexed4CandidateDebugImageErrorCode::InvalidPlaneDimensions));
  }
  ...
  if (plane.element_encoding !=
      asset::TextureTransferElementEncoding::Packed4) {
    return std::unexpected(Error(TdxIndexed4CandidateDebugImageErrorCode::
  ...
  if (!Multiply(storage.width, storage.height, texel_count) ||
      !Add(texel_count, 1U, rounded_texel_count) ||
      !Add(rounded_texel_count / 2U, kPaletteBytes, source_bytes)) {
    return std::unexpected(
  ...
  if (packed_index_bytes != static_cast<std::uint64_t>(plane.bytes.size())) {
    return std::unexpected(
        Error(TdxIndexed4CandidateDebugImageErrorCode::IndexByteSizeMismatch));
  }
...
PaletteIndex(const std::uint8_t index,
             const TdxIndexed4PalettePermutationCandidate candidate) noexcept {
  if (candidate ==
...
[[nodiscard]] constexpr std::uint8_t
UnpackIndex(const std::byte packed, const std::size_t nibble_index,
            const TdxIndexed4NibbleOrderCandidate candidate) noexcept {
  const std::uint8_t value = std::to_integer<std::uint8_t>(packed);
  const bool first_nibble = (nibble_index & 1U) == 0U;
  const bool use_low =
      candidate == TdxIndexed4NibbleOrderCandidate::LowNibbleFirst
          ? first_nibble
          : !first_nibble;
  return use_low ? static_cast<std::uint8_t>(value & 0x0fU)
                 : static_cast<std::uint8_t>(value >> 4U);
}

[[nodiscard]] constexpr std::array<std::size_t, 3>
SourceChannelSlots(const TdxIndexed4SourceChannelCandidate candidate) noexcept {
  switch (candidate) {
  case TdxIndexed4SourceChannelCandidate::SourceSlots012:
    return {0U, 1U, 2U};
  case TdxIndexed4SourceChannelCandidate::SourceSlots021:
  ...
  try {
    ...
    for (std::size_t output_y = 0U; output_y < height; ++output_y) {
      ...
      for (std::size_t x = 0U; x < width; ++x) {
        ...
        const std::uint8_t unpacked_index = UnpackIndex(
            plane.bytes[source_index / 2U], source_index, policy.nibble_order);
        const std::uint8_t palette_index =
            PaletteIndex(unpacked_index, policy.palette_permutation);
    ...
    return std::unexpected(
        Error(TdxIndexed4CandidateDebugImageErrorCode::AllocationFailed));
