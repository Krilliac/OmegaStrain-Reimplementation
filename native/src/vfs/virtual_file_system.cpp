...
[[nodiscard]] std::expected<std::string, std::string> SafeGenericString(
    const std::filesystem::path& path)
{
    ...
    {
        return std::unexpected("mounted filename cannot be represented for normalization");
    }
}
    ...
    const std::uint64_t maximum_bytes)
{
    if (size > maximum_bytes)
        return std::unexpected("file range exceeds caller's read limit");
    if (size > static_cast<std::uint64_t>(std::numeric_limits<std::size_t>::max()) ||
        ...
        return std::unexpected("file range is too large for this process");

    std::ifstream stream(path, std::ios::binary);
    if (!stream)
        return std::unexpected("unable to open mounted file");
    stream.seekg(static_cast<std::streamoff>(offset), std::ios::beg);
    if (!stream)
        return std::unexpected("unable to seek mounted file");

    std::vector<std::byte> bytes(static_cast<std::size_t>(size));
    if (!bytes.empty() && !stream.read(reinterpret_cast<char*>(bytes.data()), static_cast<std::streamsize>(bytes.size())))
        return std::unexpected("unable to read complete mounted file range");
    return bytes;
}
...
public:
        ...
        const std::filesystem::path& root)
    {
        ...
        if (error || !is_directory)
            return std::unexpected("mount root is not a readable directory");
             ...
             iterator != end && !error; iterator.increment(error))
        {
            ...
            if (error)
                break;
            auto relative_text = SafeGenericString(relative);
            if (!relative_text)
                return std::unexpected(relative_text.error());
            auto normalized = NormalizeGamePath(*relative_text);
            if (!normalized)
                return std::unexpected("unable to normalize mounted path");
            ...
            if (!mount->files_.emplace(std::move(*normalized), iterator->path()).second)
                return std::unexpected(
                    "directory contains duplicate case-insensitive game path");
        }
        if (error)
...
private:
    ...
    Resolve(const std::string_view normalized_path) const
    {
        ...
        if (error)
            return std::unexpected("unable to determine mounted file size");
