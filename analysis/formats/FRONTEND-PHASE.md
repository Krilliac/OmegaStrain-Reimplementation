...
Status: Python contract, generated fixture, and independent test-only C++ wire
mirror. No `OMEGAFRPHASE0002` PCSX2 producer, owner capture, or runtime
consumer exists yet.

`tools/assemble_frontend_phase.py` validates the fixed-width
`OMEGAFRPHASE0002` research envelope. An owner-produced envelope and its
normalized detailed trace are private intermediates. They belong only at
explicitly selected, ignored destinations. They are not evidence-ledger
artifacts and must not be committed after an owner-data run. The tracked
`synthetic-producer.hex` is project-generated test data, not an owner capture.
...
## Independent test-only wire mirror (E-0128)

`native/tests/frontend_phase_wire_contract_tests.cpp` is a standalone,
test-only C++ encoder. It independently constructs the generated version-2
model as owned bytes and compares the result with the exact 495-byte tracked
fixture. It has no OpenOmega library dependency, exported header, shipping
target edge, decoder, canonical-IR type, runtime consumer, or owner-data
integration.

`analysis/fixtures/frontend_phase_v2/synthetic-contract.txt` is a shared,
ASCII-only mechanical contract for the magic, version, header width, table
order, row widths, fixture byte count, and all twenty immutable hard-limit
names and values. The C++ test and Python contract tests consume that file
independently so source/fixture drift fails visibly. The test also covers the
wire enum and failure-counter mappings, table offsets, deterministic owned
output, each exact hard ceiling and ceiling-plus-one rejection, and the exact
versus one-below fragment-output bound.

This mirror establishes only an independent implementation check over
project-generated bytes. It is not a producer or independent retail evidence.
The CMake target is registered, but local C++ compilation and execution remain
pending until the host preflight permits a serialized build.

## Mandatory next blockers

The generated Python builder/parser, checked-in fixture, and C++ mirror are not
producer evidence. Before any owner capture:

1. the registered C++ mirror must pass a serialized native build and executable
   test;
2. the PCSX2 producer must preserve immutable submission-time context and
   terminal reconciliation;
3. the scene and front-end artifacts must gain the shared capture-domain field
   and the private combined validator must be exercised across all three
   artifacts; and
...
No phase-format producer, owner capture, named phase, grounded
layer/text/animation order, retail menu, frame parity, or PCSX2 equivalence is
claimed here.
