# .sub — occurrence-only archive-member dossier

## 1. Identity

The suffix `.sub` is observed as a HOG member but has no dedicated structural aggregate,
...
| --- | ---: | --- |
| Recursive HOG inventory | 42 | `analysis/formats/asset-fingerprints.json` |
| Top-level HOG inventory | 42 | `analysis/formats/hog-validation.json` |
| Whole-disc inventory | 0 | `analysis/manifests/disc-summary.json` |

## 3. Confirmed facts

- The tracked inventories establish 42 recursive and 42 direct top-level HOG
...
- `analysis/formats/DECODER-COVERAGE.md` classifies `.sub` as
