# .skf — occurrence-only archive-member dossier

## 1. Identity

The suffix `.skf` is observed as a HOG member but has no dedicated structural aggregate,
...
| --- | ---: | --- |
| Recursive HOG inventory | 26 | `analysis/formats/asset-fingerprints.json` |
| Top-level HOG inventory | 26 | `analysis/formats/hog-validation.json` |
| Whole-disc inventory | 0 | `analysis/manifests/disc-summary.json` |

## 3. Confirmed facts

- The tracked inventories establish 26 recursive and 26 direct top-level HOG
...
- `analysis/formats/DECODER-COVERAGE.md` classifies `.skf` as
