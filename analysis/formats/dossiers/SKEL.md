# .skel — occurrence-only archive-member dossier

## 1. Identity

The suffix `.skel` is observed as a HOG member but has no dedicated structural aggregate,
...
| --- | ---: | --- |
| Recursive HOG inventory | 4 | `analysis/formats/asset-fingerprints.json` |
| Top-level HOG inventory | 4 | `analysis/formats/hog-validation.json` |
| Whole-disc inventory | 0 | `analysis/manifests/disc-summary.json` |

## 3. Confirmed facts

- The tracked inventories establish 4 recursive and 4 direct top-level HOG
...
- `analysis/formats/DECODER-COVERAGE.md` classifies `.skel` as
