...
4. **No tracked adversarial/fuzz test of the TDX decoder against malformed or resource-boundary input is documented as run** (see Section 7 for detail) beyond the implicit-zero-suffix allowlist and the standard corpus verifier's zero-error result. A privacy-safe collection is a synthetic malformed-header/oversized-`block_stride`/truncated-block fuzz corpus (generated, not extracted from any disc) exercised through the existing native decoder and its typed-error path.
...
- **Adversarial/resource-boundary test gap:** tracked ledger entries document zero-error runs against the *owned corpus* (E-0022, E-0038, E-0043) and a proven, narrowly-scoped implicit-zero-suffix allowlist (`tools/prove_tdx_zero_suffix.py`, `TDX.md` "Proven implicit-zero suffix family") that rejects every other short form. No tracked ledger entry or test file documents a dedicated fuzz/malformed-input corpus (truncated headers, oversized `block_count`/`block_stride`, corrupt palette-plane counts) run against `tdx_texture_storage_decoder.cpp` beyond what the corpus verifier and the explicit zero-suffix allowlist already cover. This is the clearest tooling gap for this family.
...
1. **(Highest priority) Build and register a synthetic adversarial/fuzz test suite for `tdx_texture_storage_decoder.cpp`.** Generate malformed headers (bad `version`, oversized `block_count`/`block_stride`, truncated files, corrupt `palette_plane_count`) as project-generated fixtures (never extracted from disc), exercise the existing typed-error path, and assert exact rejection/diagnostic behavior. Register the new test in `CMakeLists.txt` next to `tdx_texture_storage_decoder_tests.cpp`. Closes the Section 7 gap directly.
