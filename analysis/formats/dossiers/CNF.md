# .cnf — whole-disc disposition dossier

## 1. Identity

The suffix `.cnf` is observed only in the tracked whole-disc inventory. It has no occurrence
...
| Whole-disc inventory | 8 | `analysis/manifests/disc-summary.json` |
...
- Native `GameDataService` validates the specific root `SYSTEM.CNF` BOOT2 contract used to identify a game-data root. That narrow compatibility behavior does not establish one grammar for all eight `.cnf` files. Source: `native/src/content/game_data_service.cpp` and its generated tests.

## 4. Aggregate-only facts

The only family-wide public aggregate established here is an occurrence count of 8 at
...
- **H1 — one structural family.** Files sharing `.cnf` may implement one grammar, or the
