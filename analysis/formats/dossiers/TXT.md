...
also one of the suffixes explicitly named in the frozen output
...
| C5 | `analysis/formats/FRONTEND-TOPOLOGY.md` explicitly names `.txt` as an "approved public suffix" tracked by `tools/measure_frontend_hog_topology.py`, and that tool's source maps `.txt` to the fixed category label `"text"` in its `APPROVED_EXTENSION_CATEGORIES` dictionary. This is a declared measurement-vocabulary fact only — no result JSON from a run of that tool is checked into the repository, so no actual `.txt` topology count/bucket value is available from it. | `analysis/formats/FRONTEND-TOPOLOGY.md` (approved-suffix list); `tools/measure_frontend_hog_topology.py` (`APPROVED_EXTENSION_CATEGORIES[".txt"] == "text"`) |
