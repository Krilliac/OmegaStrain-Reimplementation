...
- `InspectGuiEnvelope` validates only the bounded project-defined prefix hypothesis, reports neutral
  byte ranges and one neutral word, and deliberately does not traverse or validate the opaque root.
  Generated fixtures confirm code behavior, not retail provenance.
...
- E-0113's separate schema-version-1 `frontend-envelope-coverage-verify-tree` command can count
  project-passive descriptor acceptance for HOG-contained GUI candidates. Its generated fixtures
  are implementation evidence; no owner-corpus coverage result is recorded.
