# .map — whole-disc disposition dossier

## 1. Identity

The suffix `.map` is observed only in the tracked whole-disc inventory. It has no occurrence
...
- **H1 — one structural family.** Files sharing `.map` may implement one grammar, or the
