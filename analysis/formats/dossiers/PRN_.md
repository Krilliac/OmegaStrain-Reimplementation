# .prn — occurrence-only archive-member dossier

## 1. Identity

The suffix `.prn` is observed as a HOG member but has no dedicated structural aggregate,
...
| --- | ---: | --- |
| Recursive HOG inventory | 1 | `analysis/formats/asset-fingerprints.json` |
| Top-level HOG inventory | 1 | `analysis/formats/hog-validation.json` |
| Whole-disc inventory | 0 | `analysis/manifests/disc-summary.json` |

## 3. Confirmed facts

- The tracked inventories establish 1 recursive and 1 direct top-level HOG
...
- `analysis/formats/DECODER-COVERAGE.md` classifies `.prn` as
  `aggregate_scanner_only`: no dedicated native boundary is claimed.
- This dossier uses `PRN_.md` because `PRN` is a reserved Windows device name. Any future grammar note must retain a Windows-safe basename.
