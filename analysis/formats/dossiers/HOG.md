...
- **H1 — `tag` is a checksum or archive-identity value, not a magic constant.** The two cited example tags in tracked docs (`0x4052673D` for the MINSK `SCRIPTS.HOG` example in `analysis/formats/HOG.md`) vary per archive, which is inconsistent with a fixed magic number but consistent with a per-archive checksum, build timestamp, or content hash. *Confirming/refuting observation (privacy-safe):* an aggregate script over the tracked corpus that buckets the `tag` field's distinct-value count against the 273 top-level archive count and the 6,677 nested-archive count — if distinct-value count ≈ archive count, a checksum/identity role is more plausible than a shared magic; this requires no payload disclosure, only a count published to `asset-fingerprints.json`.
- **H2 — nested-HOG zero-tail padding of `[16, 2032]` bytes reflects sector/alignment rounding of the parent container, not a payload-format requirement.** *Confirming/refuting observation:* an aggregate check correlating each nested HOG's zero-padding byte count against its parent top-level archive's on-disk sector size (already known to be CD/DVD-sector-based per the whole-disc manifest) — publishable as an aggregate histogram of "padding bytes mod sector size" with no filenames.
...
- **No published aggregate breakdown of the `tag` field's value distribution.** Only two example tag values appear in tracked markdown (the MINSK `SCRIPTS.HOG` case in `HOG.md`); there is no corpus-wide histogram or uniqueness count for `tag` across the 273 top-level or 6,677 nested archives in `asset-fingerprints.json` or `hog-validation.json`. *Privacy-safe collection:* extend `tools/validate_hogs.py` or `tools/fingerprint_assets.py` to emit an aggregate (count of distinct values, min/max, or a bucketed histogram) for the `tag` field, written to the existing JSON outputs — no per-file tag-to-name mapping needed.
