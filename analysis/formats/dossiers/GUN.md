# .gun — occurrence-only archive-member dossier

## 1. Identity

The suffix `.gun` is observed as a HOG member but has no dedicated structural aggregate,
...
| --- | ---: | --- |
| Recursive HOG inventory | 624 | `analysis/formats/asset-fingerprints.json` |
...
- The tracked inventories establish 624 recursive and 0 direct top-level HOG
  occurrences, with 0 whole-disc files.
- Subtracting the direct count from the recursive count establishes 624 occurrences reached
  through nested HOG descent. This is a count relation only; it assigns no role.
- `analysis/formats/DECODER-COVERAGE.md` classifies `.gun` as
...
this suffix. The hardened size-only member collector permits this suffix only through its explicit optional allowlist. Its implementation and synthetic tests do not become format evidence until a sanitized owner-corpus result is reviewed.
